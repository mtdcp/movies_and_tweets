...
export default class Movies extends Component {
    ...
    getMovies() {
        ...
        axios.get('https://api.themoviedb.org/3/search/person?api_key=<apiKey>&query=' + document.getElementById("txtbusca").value).then(data => {   
            this.setState({ movies: data.data.results })

            this.state.movies.forEach(movie => {
                movie.known_for.forEach(subMovie => {
                    axios.get('http://www.omdbapi.com/?t=' + subMovie.title + '&apikey=<apiKey>').then(data => {   
